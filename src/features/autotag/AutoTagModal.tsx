...
/**
 * Review-before-apply UI for audio auto-tagging. On open it looks the file up in
 * the Apple Music catalogue (by its filename), lists the matching versions to
 * pick from, lets the user edit any field or search manually, then writes the
 * tags + cover on Apply.
 */
export function AutoTagModal({ path, filename, onClose }: AutoTagModalProps) {
    ...
    identifyAudio(path, controller.signal)
      .then((data) => {
        if (data.results.length > 0) {
          ...
          // Nothing found — seed the manual search from the filename.
      ...
      >
        ...
        {stage === "done" && (
          <div className="py-12 flex flex-col items-center gap-3">
            ...
            <span className="text-sm text-zinc-200">{t("autotag.applied")}</span>
          </div>
        )}
        ...
        {reviewing && (
          <>
            {error && (
              <p className="text-xs text-amber-400/90 mb-3">{error}</p>
            )}
              ...
              <div className="w-28 h-28 rounded-xl overflow-hidden bg-white/5 shrink-0 flex items-center justify-center">
                {coverUrl ? (
                  <Thumbnail
                    ...
                    fallback={<Music4 className="size-8 text-white/40" />}
                  />
                ) : (
                  <Music4 className="size-8 text-white/40" />
                )}
              </div>

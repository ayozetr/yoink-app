...
def _build_options(
    ...
    *,
    ...
    # Auto PO token: in "auto" mode a YouTube download mints a session GVS token
    # (via the WebView bridge) and passes it plus the visitor_data it's bound to,
    # overriding the generic po_token network_options() set. No-op for non-YouTube
    # URLs and for manual/off mode. Runs on the download worker thread; the mint is
    # cached per session, so the cookie fallback's second _build_options call is instant.
    from app.services import po_token

    yt_args = po_token.youtube_extractor_args(str(request.url))
    if yt_args:

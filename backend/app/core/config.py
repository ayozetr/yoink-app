...
class Settings(BaseSettings):
    ...
    # Loudness-normalize audio downloads (EBU R128) so every track plays at the
    # same volume. Off by default (it re-encodes the audio).
    normalize_audio: bool = False
    # Target integrated loudness in LUFS when normalize_audio is on. -14 is the
    # streaming standard (Spotify/YouTube); -16 ≈ Apple Music; -23 = broadcast.
    normalize_lufs: int = -14
